...
import { atomicWriteFile, updateOperationCheckout, writeOperation } from './operations.js';
import { parseSidecar, renderSidecar, restoredSidecarBatches } from './sidecar.js';
import { readFile } from 'node:fs/promises';
import path from 'node:path';
const NOTE_START = /^## @.+?#\d+(?:-\d+)?\s*$/m;
/** Separates exact note bytes from a sidecar without interpreting individual notes. */
export function splitSidecarNotes(raw, sidecarPath = '.grind.md') {
    ...
    if (match === null)
        return { withoutNotes: raw, payload: '' };
...
export function appendSidecarNotes(raw, initiative, payload) {
    if (payload === '')
        return renderSidecar(raw, initiative);
...
function bodyPrefix(raw) {
    const parsed = parseFrontmatter(raw);
    if (parsed.error !== undefined)
        throw new GrindError('ARTIFACT_INVALID', parsed.error);
    return { prefix: raw.slice(0, raw.length - parsed.body.length), body: parsed.body };
}
function markers(operationId) {
    if (!/^[A-Za-z0-9._-]+$/.test(operationId))
        throw new GrindError('OPERATION_INVALID', 'Operation id is not safe for a note marker');
    return {
...
export function parkNotesInLedger(raw, repository, operationId, payload) {
    if (payload === '')
        return raw;
    if (/\r|\n/.test(repository))
        throw new GrindError('NOTE_INVALID', 'Repository path cannot contain a newline');
...
export function restoreNotesFromLedger(raw, operationId) {
    const { prefix, body } = bodyPrefix(raw);
    const marker = markers(operationId);
    const start = body.indexOf(marker.start);
    if (start === -1)
        return { ledger: raw, payload: '' };
    ...
    if (end === -1)
        throw new GrindError('ARTIFACT_INVALID', `Parked note batch ${operationId} has no closing marker`);
...
export function listParkedNoteBatches(raw, repository) {
 ...
 */
export async function parkCheckoutNotes(workspace, operation, checkoutIndex, sourceLedgerPath) {
    ...
    if (checkout.notePayload === undefined) {
        const sidecarRaw = await readFile(sidecarPath, 'utf8').catch((error) => {
        ...
        checkout = current.checkouts[checkoutIndex];
    }
    const payload = checkout.notePayload;
    if (payload === '') {
    ...
    if (checkout.noteState === 'captured') {
        ...
        if (parked !== ledgerRaw)
            await atomicWriteFile(sourceLedgerPath, parked);
    ...
    if (checkout.noteState === 'parked') {
        ...
        if (sidecarRaw !== '') {
            ...
            if (split.payload !== '')
                await atomicWriteFile(sidecarPath, split.withoutNotes);
        }
        current = updateOperationCheckout(current, checkoutIndex, { noteState: 'removed' }, `notes:${checkout.repository}:removed`);
        await writeOperation(workspace, current);
    }
...
export async function restoreCheckoutNotes(workspace, operation, checkoutIndex, targetLedgerPath, targetInitiative) {
    let current = operation;
    let checkout = current.checkouts[checkoutIndex];
    if (checkout === undefined)
        throw new GrindError('OPERATION_INVALID', `Operation has no checkout at index ${checkoutIndex}`);
    ...
    const batches = checkout.restoreBatches;
    if (checkout.restoreState === 'captured') {
        ...
        for (const batch of batches) {
            if (ids.includes(batch.operationId))
                continue;
        ...
        if (next !== raw)
            await atomicWriteFile(sidecarPath, next);
    ...
    if (checkout.restoreState === 'copied') {
        ...
        if (nextLedger !== ledgerRaw)
            await atomicWriteFile(targetLedgerPath, nextLedger);
    ...
    if (checkout.restoreState === 'removed') {
        ...
        if (cleaned !== raw)
            await atomicWriteFile(sidecarPath, cleaned);
