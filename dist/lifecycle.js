...
import { branchOwners } from './resolve.js';
import { git } from './git.js';
import { inspectInitiative } from './inspect.js';
import { withLifecycleLocks } from './operations.js';
...
async function inProgressOperation(checkout) {
...
async function branchOccupiedElsewhere(checkout, branch) {
...
async function planTargetSource(checkout, targetBranch, blockers, repository) {
    const local = await git(['show-ref', '--verify', '--quiet', `refs/heads/${targetBranch}`], checkout);
    if (local.ok)
        return 'local';
    const remoteMatches = await git(['for-each-ref', '--format=%(refname:short)', `refs/remotes/*/${targetBranch}`], checkout);
...
async function targetReference(checkout, branch, source) {
    if (source === 'local')
...
async function inspectCheckout(workspace, target, repository, blockers) {
    const { recorded, observed } = repository;
    const action = recorded.checkout !== 'clone' ? 'verify-worktree' : repository.onRecordedBranch ? 'stay' : 'switch';
    const planned = {
    ...
    if (!observed.exists || !observed.repository)
        return planned;
    if (observed.detached)
        blockers.push(`${recorded.path} has a detached HEAD`);
    const operation = await inProgressOperation(observed.path);
    if (operation !== null)
        blockers.push(`${recorded.path} has an in-progress Git operation (${operation})`);
    ...
    if (recorded.checkout !== 'clone') {
        if (!repository.onRecordedBranch)
            blockers.push(`${recorded.path} worktree is on ${observed.branch}, not ${recorded.branch}`);
        return planned;
    }
    if (repository.onRecordedBranch)
        return planned;
    ...
    if (trackedSidecar.ok)
        blockers.push(`${recorded.path} tracks .grind.md; remove it from repository history before switching`);
    ...
    if (occupied !== null)
        blockers.push(`${recorded.path} branch ${recorded.branch} is checked out at ${occupied}`);
    return planned;
}
/** Builds a complete, read-only checkout plan. Execution revalidates it after locks are acquired. */
export async function planStart(workspace, inspection) {
    const blockers = inspection.diagnostics
        .filter((diagnostic) => diagnostic.severity === 'error')
        .map((diagnostic) => diagnostic.message);
    if (inspection.state === null)
        blockers.unshift('ledger state is malformed or legacy');
    const checkouts = [];
...
export async function withLockedStartPlan(workspace, inspection, operationId, action) {
