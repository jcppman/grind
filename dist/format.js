...
function initiativeHeading(id, status, options) {
    if (!useColor(options))
        return `${id}  [${status}]`;
...
function field(label, value, width, indent = '  ') {
...
function diagnosticsBlock(diagnostics, width, indent = '  ') {
    return diagnostics.flatMap((diagnostic) => field(diagnostic.severity === 'error' ? 'Error' : 'Warning', `${diagnostic.code}: ${diagnostic.message}`, width, indent));
}
function listSummary(result) {
    ...
    if (counts.size === 1) {
        const [status, count] = counts.entries().next().value;
...
export function formatList(result, options = {}) {
    ...
    for (const item of result.initiatives) {
        ...
        if (item.current_task)
            lines.push(...field('Task', item.current_task, width));
        if (item.next_action)
            lines.push(...field('Next', item.next_action, width));
        if (item.result)
            lines.push(...field('Result', item.result, width));
...
function formatInspection(inspection, width, options) {
    const lines = [];
    ...
    if (inspection.repositories.length > 0)
    ...
    for (const repo of inspection.repositories) {
        ...
        if (!observed.exists)
            seen = 'missing';
        else if (!observed.repository)
            seen = 'not a git checkout';
        else
            seen = observed.detached ? 'detached HEAD' : `on ${observed.branch}`;
...
export function formatStart(result, options = {}) {
    const width = outputWidth(options);
    const lines = formatInspection(result.inspection, width, options);
    ...
    if (result.pendingNotes > 0)
        lines.push(...wrap(`${result.pendingNotes} review note(s) await handling before resuming.`, width));
    return lines.join('\n');
}
