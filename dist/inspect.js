...
import { gitChangedFiles, gitCurrentBranch, gitToplevel } from './git.js';
...
async function inspectRepository(workspace, entry, recorded, diagnostics) {
    ...
    observed.changedFiles = ((await gitChangedFiles(checkoutPath)) ?? []).filter((line) => porcelainPath(line) !== SIDECAR_FILENAME);
...
export function startBlockers(inspection) {
    const blockers = [];
    if (inspection.state === null)
        blockers.push('ledger state is malformed or legacy');
    ...
    for (const repo of inspection.repositories) {
        if (repo.observed.exists && repo.observed.repository && !repo.onRecordedBranch) {
            blockers.push(repo.observed.detached
