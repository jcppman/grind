...
import { findWorkspaceConfig, loadWorkspace, readWorkspaceSettings } from './workspace.js';
...
function command(pluginRoot, workspace, id) {
...
function boundedNotice(notice, budget = HOOK_CONTEXT_BYTES) {
...
export function presentHookContext(result, pluginRoot, budget = HOOK_CONTEXT_BYTES) {
    ...
    if (Buffer.byteLength(full, 'utf8') <= budget)
        return full;
...
export async function sessionContext(input, pluginRoot) {
    let workspace;
    try {
        ...
        if (!settings.contextOnSessionStart)
            return null;
        ...
        if (error instanceof GrindError && error.code === 'INITIATIVE_UNRESOLVED' &&
            ...
            if (!sidecar?.initiative && !sidecar?.diagnostics.length)
                return null;
        }
