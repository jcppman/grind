...
interface Heading { slug: string; text: string; depth: number; start: number; end: number }
...
export function markdownSection(body: string, fragment: string): string {
  const heading = headings(body).find(entry => entry.slug === fragment);
  if (!heading) throw new Error(`Required fragment #${fragment} does not exist`);
  return body.slice(heading.start, heading.end);
}

export function requiredLinks(body: string): string[] {
  const tokens = marked.lexer(body);
  const sections = tokens.flatMap((token, index) =>
    token.type === 'heading' && token.depth === 2 && inlineText(token.tokens ?? []) === 'Read on every context load' ? [index] : []);
  if (sections.length > 1) throw new Error('Multiple Read on every context load sections are ambiguous');
  ...
  for (const token of section) {
    if (token.type === 'heading' || token.type === 'space' || token.type === 'def') continue;
    if (token.type !== 'list') throw new Error('Required reads must be Markdown links in list items');
    for (const item of token.items) {
      const found: string[] = [];
      marked.walkTokens(item.tokens, child => { if (child.type === 'link') found.push(child.href); });
      if (found.length === 0) throw new Error('Required read list item has no supported Markdown link');
      links.push(...found);
    }
  }
  return links;
...
export interface ContextLink { label: string; path: string; fragment: string | null; external: boolean }

export function resolveLink(href: string, source: string): Omit<ContextLink, 'label'> {
...
export function navigation(body: string, source: string): ContextLink[] {
  const links: ContextLink[] = [];
  marked.walkTokens(marked.lexer(body), token => {
    if (token.type === 'link') links.push({ label: inlineText(token.tokens ?? []), ...resolveLink(token.href, source) });
  });
  return links;
}
