...
export async function addAgentNote(input: AgentNoteInput): Promise<{ sidecar: string; reference: string }> {
  ...
  if (input.comment.trim() === '') throw new GrindError('NOTE_INVALID', 'Comment must not be empty');
  ...
  if (repositoryRoot === null) throw new GrindError('NOTE_INVALID', `File is not inside a Git repository: ${input.file}`);
  ...
  return withRepositoryLock(repositoryRoot, `agent-note-${process.pid}`, async () => {
    ...
    const anchor = lines[input.start - 1] as string;
...
export async function runAgentNote(argv: readonly string[]): Promise<number> {
  try {
    ...
    const comment = argv[hasEnd ? 3 : 2] as string;
    const result = await addAgentNote({ file: file as string, start: Number(startText), end: Number(endText), comment });
    process.stdout.write(`${result.reference} -> ${result.sidecar}\n`);
    return 0;
  } catch (error) {
    const failure = error instanceof GrindError ? error : new GrindError('NOTE_INVALID', (error as Error).message);
