...
const build = process.env.GRIND_BUILD;
if (build && !/^[0-9A-Za-z.-]+$/.test(build)) throw new Error('GRIND_BUILD must be valid semver build metadata');
// Hosts cache installs by version string, so each published build needs a distinct one.
const version = build ? `${pkg.version}+${build}` : pkg.version;
...
await writeFile(path.join(output, '.codex-plugin', 'plugin.json'), JSON.stringify({ ...codexManifest, version }, null, 2) + '\n');
await writeFile(path.join(output, '.claude-plugin', 'plugin.json'), JSON.stringify({ ...claudeManifest, version }, null, 2) + '\n');
await mkdir(path.join(output, 'scripts'));
await cp(path.join(root, 'scripts', 'grind.mjs'), path.join(output, 'scripts', 'grind.mjs'));
await writeFile(path.join(output, 'package.json'), JSON.stringify({ ...pkg, version, scripts: undefined, devDependencies: undefined }, null, 2) + '\n');
...
process.stdout.write(`Plugin ${version}: ${output}\n`);
